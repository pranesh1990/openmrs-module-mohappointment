...
public class DWRAppointmentUtil {
	...
	public String getPatientListInTable(String searchString, String id) {
		...
		if (appointments != null) {
			sb.append(
					"<table class='openmrsSearchTable' cellpadding=2 cellspacing=0 style='width:100%; font-size:0.8em'>");
			sb.append("<tr><td colspan='12' style='text-align:right; font-style:italic;'>Results for &quot;" + id
					+ "&quot;: " + appointments.size() + " appointments</tr>");
			sb.append(
					"<tr class='oddRow'><th>#</th><th>Identifier</th><th>Patient Names</th><th>Age</th><th>Gender</th><th></th><th>Birthdate</th><th>Appointment Date</th><th>Provider</th><th>Reason of Appointment</th><th>State</th><th></th><th></th></tr>");
			i = 0;
			for (Integer appId : appointments) {
				...
				sb.append("<td onclick=showDialog('" + app.getAppointmentId() + "','" + name.replace(" ", "&nbsp;")
						+ "','" + provName.replace(" ", "&nbsp;") + "','" + appDate + "','"
						+ reason.replace(" ", "&nbsp;") + "',1)><input type='button' value='WAITING/INADVANCE'/></td>");
				sb.append("<td onclick=showDialog('" + app.getAppointmentId() + "','" + name.replace(" ", "&nbsp;")
						+ "','" + provName.replace(" ", "&nbsp;") + "','" + appDate + "','"
						+ reason.replace(" ", "&nbsp;") + "',2)><input type='button' value='POSTPONE'/></td>");
				sb.append("</tr>");
			}
			sb.append("</table>");
		}
